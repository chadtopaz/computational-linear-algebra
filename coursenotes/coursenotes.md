...
`{= latex} \renewcommand{\vec}[1]{\bm{#1}} \renewcommand{\mat}[1]{\bm{#1}} \newcommand{\exp}[1]{\mathrm{e}^{#1}} \DeclareMathOperator*{\argmin}{arg\,min}`
...
2.  Dot product, also known as the inner product. To compute, calculate
    *x⃗* ⋅ *y⃗* ≡ *x⃗*<sup>*T*</sup>*y⃗* where the superscript *T* indicates
...
3.  Outer product. To compute, calculate
    $\vec{x} \otimes \vec{y} \equiv \vec{x}\hphantom \vec{y}^T$. Note
    that *x⃗* and *y⃗* do not need to have the same number of elements,
    and the result is a matrix. If *x⃗* is *m* × 1 and *y⃗* is *n* × 1
    then \]*x⃗* ⊗ *y⃗* is *m* × *n*.
