...
3.  Outer product. To compute, calculate $\vec{x} \otimes \vec{y} \equiv \vec{x}\hphantom \vec{y}^T$. Note that $\vec{x}$ and $\vec{y}$ do not need to have the same number of elements, and the result is a matrix. If $\vec{x}$ is $m \times 1$ and $\vec{y}$ is $n \times 1$ then $]\vec{x} \otimes \vec{y}$ is $m \times n$.
