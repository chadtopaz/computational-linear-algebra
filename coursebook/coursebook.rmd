...
By the way, we can go ahead and use the `echelon` command from the `matlib` library to perform the elimination.
```{r cache = TRUE}
...
$$
\frac{\mathrm{relative forward error}}{\mathrm{relative backward error}}=\frac{\frac{||\vec{x}-\vec{x}_a||}{||\vec{x}||}}{\frac{||\vec{b}-\mat{A}\vec{x}_a||}{||\vec{b}||}}.
$$
